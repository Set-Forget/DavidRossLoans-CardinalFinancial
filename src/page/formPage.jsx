...
export default function FormPage() {
  ...
  const userFromList = usersList.find(
    (userFromList) => user.email === userFromList.email
  );
  ...
  const filteredForms = forms.filter((form) => {
    const matchesName = form.formName
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesDealId = form.dealId.toString().includes(searchTerm);
    return matchesName || matchesDealId;
  });

  const totalPages = Math.ceil(filteredForms.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedData = filteredForms.slice(startIndex, endIndex);
